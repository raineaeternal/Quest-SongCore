...
fn load_song_from_zip_and_directory_match() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Load from zip
    let song_from_zip = song_load_rs::song_loader::load_song_from_path(zip_path.clone(), None)
        .map_err(|e| format!("load from zip failed: {}", e))?;

    // Load from directory (the public API accepts both file and directory)
    let song_from_dir = song_load_rs::song_loader::load_song_from_path(dir_path.clone(), None)
...
fn load_song_directory_finds_songs() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let loaded = song_load_rs::song_loader::load_song_directory(
        &tests_dir,
        None,
        None::<fn(&song_load_rs::song_loader::LoadedSong, usize, usize)>,
    )
...
fn load_song_with_memcache() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let loaded_dir = song_load_rs::song_loader::load_song_directory::<
        fn(&song_load_rs::song_loader::LoadedSong, usize, usize),
